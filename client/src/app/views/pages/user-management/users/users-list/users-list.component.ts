...
export class UsersListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['select', 'username', 'email', 'firstName', '_roles', 'actions'];
	 ...
	 */
	ngOnInit() {
		console.log("IN UserListComponent: >>>>>>> ");
		// load roles list
	 ...
	 */
	fetchUsers() {
		console.log("IN fetchUsers: >>>> ", this.selection);
		const messages = [];
	...
	getUserRolesStr(user: User): string {
		const names: string[] = [];
		/*
		each(user.roles, (roleId: any) => {
			const _role = find(this.allRoles, (role: Role) => role.id === roleId);
			if (_role) {
				names.push(_role.name);
			}
		});
		*/
		each(user.roles, (_role: Role) => {
			if (_role) {
				names.push(_role.name);
			}
		});
