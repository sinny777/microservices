...
import { mergeMap, map, withLatestFrom, filter, tap, retry, catchError } from 'rxjs/operators';
// NGRX
    ...
    loadAllRoles$ = this.actions$
        .pipe(
            ...
            map(response => {
                ...
                console.log('ALL ROLES LOADED, Result: ', result);
                return new AllRolesLoaded({roles: result.items});
            })
          );
